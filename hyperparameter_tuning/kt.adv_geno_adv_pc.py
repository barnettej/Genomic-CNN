...
def build_model(hp):
    ...
    pheno_out_geno_gr = GradientReversalLayer(hp.Float(name = "gr_weight_geno", min_value=0.001, max_value=0.2, sampling = "log"))(x_geno)
    pheno_out_geno = Dense(1, activation="sigmoid", name="pheno_out_geno", kernel_initializer=tf.keras.initializers.GlorotUniform())(pheno_out_geno_gr)
...
tuner = Hyperband(
    build_model,
    objective=keras_tuner.Objective("val_pheno_out_auc", direction="max"),
    # max_trials=trials,
    # executions_per_trial=repeats,
    project_name="kt.adv_geno_adv_pc.11.11.22.2",
