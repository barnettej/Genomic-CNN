...
def build_model(hp):
    conv_blocks = hp.Int("conv_blocks", 1,3)
    ...
    for k in range(conv_blocks):
        print(x.shape)
        with hp.conditional_scope(parent_name="conv_blocks", parent_values=list(range(k + 1, conv_blocks + 1))):
            filters = hp.Int("conv_block_filters" + str(k), 10, 100)
            width = hp.Int("width" + str(k), 1, 20)
            pool_width = hp.Int("pool_width" + str(k), 1, 10)

        # run convolutional layer on the output of the multiplication
        shared_conv = ConvBlock(filters, width, pool_width)
        x = shared_conv(x)
    ...
    for k in range(dense_layers):
        ...
        shared_dense = layers.Dense(units=units, activation="relu",
                             # kernel_regularizer=regularizers.L2(L2)
                             )
        shared_drop = layers.Dropout(rate)
    ...
    pheno_out_gr = GradientReversalLayer(
        hp.Float(name="gr_weight_geno", min_value=0.001, max_value=0.2, sampling="log"))(x_geno)
    pheno_out = Dense(1, activation="sigmoid", name="pheno_out", kernel_initializer=tf.keras.initializers.GlorotUniform())(pheno_out_gr)
    ...
    pc_out_gr = GradientReversalLayer(hp.Float(name = "gr_weight_pc", min_value=0.001, max_value=0.2, sampling = "log"))(x)
...
tuner = Hyperband(
    build_model,
    objective=keras_tuner.Objective("val_pheno_out_geno_auc_1", direction="max"),
    # max_trials=trials,
    # executions_per_trial=repeats,
    project_name="kt.adv_cid_adv_pc.11.13.22.2",
