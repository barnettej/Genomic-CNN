...
shared_conv = ConvBlock(19, 6, 1)
x = shared_conv(x)
x_geno = shared_conv(x_geno)
print(x.shape)
shared_conv2 = ConvBlock(85, 11, 5)
x = shared_conv2(x)
x_geno = shared_conv2(x_geno)
print(x.shape)
...
shared_dense = layers.Dense(units=90, activation="relu",
                            )
shared_drop = layers.Dropout(0.4746)
...
shared_dense2 = layers.Dense(units=34, activation="relu",
                            )
shared_drop2 = layers.Dropout(0.2232)
...
geno_gr = GradientReversalLayer(0.18298)(x_geno)
pheno_out_geno = Dense(1, activation="sigmoid", name="pheno_out_geno", kernel_initializer=tf.keras.initializers.GlorotUniform())(geno_gr)
print("pheno out geno:", pheno_out_geno.shape)

pc_out_gr = GradientReversalLayer(0.0016139)(x)
...
np.savetxt("test.cnn_adv_geno_adv_pc.csv", predictions[0],  delimiter=",")
np.savetxt("test.cnn_adv_geno_adv_pc.geno.csv", predictions[1],  delimiter=",")
np.savetxt("test.cnn_adv_geno_adv_pc.pc_pheno.csv", predictions[3],  delimiter=",")


model.save_weights('./my_model/cnn_adv_geno_adv_pc.checkpoint')
