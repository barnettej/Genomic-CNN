...
shared_dense = layers.Dense(units=49, activation="relu",
                            )
shared_drop = layers.Dropout(0.34583854271160763)
...
shared_dense2 = layers.Dense(units=74, activation="relu",
                            )
shared_drop2 = layers.Dropout(0.4306647165450758)

x = shared_dense2(x)
x_geno = shared_dense2(x_geno)
x = shared_drop2(x)
x_geno = shared_drop2(x_geno)

pheno_gr = GradientReversalLayer(0.0012297862977242418)(x)
pheno_out = Dense(1, activation="sigmoid", name="pheno_out", kernel_initializer=tf.keras.initializers.GlorotUniform())(pheno_gr)
print("pheno out:", pheno_out.shape)

pheno_out_geno = Dense(1, activation="sigmoid", name="pheno_out_geno", kernel_initializer=tf.keras.initializers.GlorotUniform())(x_geno)
print("pheno out geno:", pheno_out_geno.shape)

pc_out_gr = GradientReversalLayer(0.013676737360624873)(x)
pc_out = Dense(10, name="pc_out", activation="linear")(pc_out_gr)
pc_to_pheno = Lambda(lambda x: K.stop_gradient(x))(pc_out)
pc_to_pheno = layers.Dense(units=84, activation="relu")(pc_to_pheno)
...
np.savetxt("test.adv_cid_adv_pc.cid_pheno.csv", predictions[0],  delimiter=",")
np.savetxt("test.adv_cid_adv_pc.geno_pheno.csv", predictions[1],  delimiter=",")
np.savetxt("test.adv_cid_adv_pc.pc.csv", predictions[3],  delimiter=",")


cagap = Model(inputs, cagap_dense2)
cagap_dense_out = cagap.predict(x = np.array(x_train), steps = train_size // batch, batch_size = batch)
np.savetxt("cagap.dense.out.train.csv", cagap_dense_out,  delimiter=",")
cagap_dense_out = cagap.predict(x = np.array(x_valid), steps = valid_size // batch, batch_size = batch)
np.savetxt("cagap.dense.out.valid.csv", cagap_dense_out,  delimiter=",")
cagap_dense_out = cagap.predict(x = np.array(x_test), steps = test_size // batch, batch_size = batch)
np.savetxt("cagap.dense.out.test.csv", cagap_dense_out,  delimiter=",")
